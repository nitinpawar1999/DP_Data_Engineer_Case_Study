from pyspark.sql.functions import sum as f_sum, col, asc, desc, lit, datediff, when, countDistinct, collect_set
...
class Question_5:
    ...
    def process(self, df_dict):
        ...
        df = df_dict['customer_df'].join(df_dict['orders_df'], 'customer_id')\
            .select('customer_city', 'customer_unique_id', 'order_id')

        df = df.join(df_dict['order_items_df'], 'order_id')\
            .select('customer_city', 'customer_unique_id', 'order_id', 'product_id')

        df = df.join(df_dict['products_df'], 'product_id')\
               .join(df_dict['product_category_name_translation_df'], 'product_category_name')\
               .select('customer_city', 'customer_unique_id', 'order_id', 'product_category_name_english')\
            .groupBy('customer_city')\
            .agg(collect_set('product_category_name_english').alias('Product_Category_Array'), 
    ...
    def export(self, final_df):
        ...
        final_df.write.mode('overwrite').parquet('output/Question5')
